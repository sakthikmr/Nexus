    ...
    <div className="space-y-10 text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-8">
          <div className="relative group">
             ...
             <input 
               ...
               className="w-full bg-white border border-slate-200 rounded-3xl py-4.5 pl-14 pr-4 text-sm font-medium focus:ring-4 focus:ring-indigo-50 focus:border-indigo-400 outline-none transition-all shadow-sm"
               value={searchQuery}
          ...
          <div className="grid grid-cols-1 gap-8">
            {filtered.map((req) => (
              <motion.div 
                layout
                key={req.id}
                className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-sm hover:border-indigo-400 transition-all group relative overflow-hidden text-left"
              >
                <div className="flex justify-between items-start mb-6 relative z-10">
                   <div className="flex items-center gap-6">
                      <div className="w-16 h-16 bg-slate-50 border border-slate-100 rounded-[1.5rem] flex items-center justify-center text-slate-300 group-hover:bg-indigo-600 group-hover:text-white transition-all shadow-inner">
                         {getTypeIcon(req.type)}
                      </div>
                      <div>
                         <div className="flex items-center gap-3 mb-2">
                            <h3 className="text-2xl font-black text-slate-900 tracking-tight group-hover:text-indigo-600 transition-colors uppercase leading-tight">{req.title}</h3>
                         ...
                         <div className="flex items-center gap-4 text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none">
                            ...
                            <span className="w-1 h-1 bg-slate-200 rounded-full" />
                            <span className="flex items-center gap-1.5"><Calendar size={14} className="text-indigo-400" /> Deadline: {req.deadline}</span>
                         </div>
                      </div>
                   </div>
                   <div className="text-right bg-slate-50 p-4 rounded-2xl border border-slate-100 shadow-inner">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Est. Budget</p>
                      <p className="text-xl font-black text-slate-900">{req.budget}</p>
                   </div>
                </div>

                <p className="text-sm text-slate-500 font-medium leading-relaxed mb-8 relative z-10 line-clamp-2 italic pr-20">
                  "{req.description}"
                </p>
                ...
                <div className="flex justify-between items-center pt-8 border-t border-slate-50 relative z-10">
                   <div className="flex items-center gap-10">
                      <div className="flex items-center gap-3">
                         <div className="flex -space-x-3">
                            {[1, 2, 3].map(i => (
                              <div key={i} className="w-9 h-9 rounded-full bg-slate-100 border-4 border-white flex items-center justify-center text-[10px] font-black text-slate-400 shadow-sm">V</div>
                            ))}
                         </div>
                         <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{req.bidCount} Active Bids</p>
                      </div>
                   ...
                   <button className="flex items-center gap-2 text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] hover:translate-x-2 transition-transform">
                      Evaluate Submissions <ArrowUpRight size={20} />
                   </button>
                </div>
        ...
        <div className="space-y-8 text-left">
           ...
           <div className="bg-slate-900 p-10 rounded-[3rem] text-white space-y-10 relative overflow-hidden shadow-2xl">
              <BarChart3 className="absolute -right-6 -bottom-6 text-white/5 scale-150" size={120} />
              <div>
                 <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 mb-8 font-mono">Engine Monitor</h4>
              ...
              <div className="space-y-4 pt-4 border-t border-white/10">
                 <button className="w-full py-4 bg-indigo-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-xl shadow-indigo-500/20 transform active:scale-95">
                    Generate Spend Report
                 </button>
                 <button className="w-full py-4 bg-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-white/20 transition-all border border-white/5 transform active:scale-95">
           ...
           <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm space-y-8 text-left">
              <div className="flex items-center gap-3 text-indigo-600">
                 <AlertCircle size={24} />
                 <h4 className="font-black uppercase text-[10px] tracking-widest italic">Governance Protocol</h4>
              </div>
              <ul className="space-y-6">
                 {[
                   'All RFPs above $50k require 3+ verified bids.',
                   'Technical evaluation must be blind for Phase 1.',
                   'Audit trail is immutable for awarded bids.'
                 ].map((tip, i) => (
                    <li key={i} className="flex gap-4 items-start">
                       <div className="mt-1.5 w-2 h-2 rounded-full bg-slate-100 border border-slate-200 shrink-0 shadow-inner" />
                       <p className="text-[11px] font-bold text-slate-500 leading-relaxed uppercase tracking-tight">{tip}</p>
                    </li>
                 ))}
              </ul>
              <div className="pt-8 border-t border-slate-50">
                 <button className="flex items-center gap-2 text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">
                    View Compliance Book <MessageSquare size={16} />
                 </button>
              </div>
           </div>
        </div>
